...
class SurveyWizard extends Wizard {
	...
	public SurveyWizard(Stage owner) {
		super(new ComplaintsPage(), new MoreInformationPage(),
				new ThanksPage());
		this.owner = owner;
	}
	...
	public void finish() {
		System.out.println(
				"Had complaint? " + SurveyData.instance.hasComplaints.get());
...
class ComplaintsPage extends WizardPage {
	...
	public ComplaintsPage() {
		...
		options.selectedToggleProperty()
	...
	Parent getContent() {
		...
		return new VBox(5, new Label("Do you have complaints?"), yes, no);
	}
...
class MoreInformationPage extends WizardPage {
	...
	Parent getContent() {
		...
		return new VBox(5, new Label("Please enter your complaints."),
				textArea);
	}
}
